...
path = mpltPath.Path(zip(xb,yb))
isin = path.contains_points(zip(x,y),radius=-np.diff(xgv)[0])
...
path = mpltPath.Path(zip(xbw,ybw))
isinw= path.contains_points(zip(x,y),radius=np.diff(xgv)[0])
...
f,R=meshlessFish.solvePoisson(x,y,s,dxzero,dyzero, xba,yba,fba,
...
plt.tricontourf(np.append(x,xbw),np.append(y,ybw),np.append(f,np.zeros(len(xbw))),
                np.linspace(-0.025,0.,11)[:-1],extend='min') # to keep tricontour from drawing in the window
plt.plot(xb,yb,'k-')
plt.plot(xbw,ybw,'k-')
plt.axis('scaled')
plt.axis('off')
